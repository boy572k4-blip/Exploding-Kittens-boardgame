...
describe("lobby actions", () => {
    ...
    it("connecting into a room", async () => {
        ...
        // wait for state sync so server-side client list is populated
        await room.waitForNextPatch();

        // make your assertions
        assert.strictEqual(client1.sessionId, room.clients[0].sessionId);
        assert.strictEqual(client1.sessionId, room.state.ownerId);
    });
...
describe("game actions", () => {
    ...
    describe("play cards", () => {
        ...
        it("play draw from bottom", async () => {
            ...
            // Ensure the bottom card is not exploding
            room.state.deck[room.state.deck.length - 1] = Card.TACOCAT;
